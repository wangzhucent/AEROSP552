...
public:
	System_Class(int freq_in, double desiredVolt_in, double desiredTime_in, double P_in, double I_in, double D_in, int window_size_in, bool isMean_in):freq(freq_in), desiredVolt(desiredVolt_in), desiredTime(desiredTime_in), P(P_in), I(I_in), D(D_in), i(0), window_size(window_size_in), isMean(isMean_in)
	{
		AthenaInit(&samples, &dscb, &dscadscan);
		gettimeofday(&start_time, NULL);
